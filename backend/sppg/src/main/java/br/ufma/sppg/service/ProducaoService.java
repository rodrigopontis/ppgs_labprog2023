...
public class ProducaoService {
    ...
    public List<Producao> obterProducoesPPG(Integer idPrograma, Integer data1, Integer data2) {
        ...
        // É Presumido que o usuário coloque em data1 o valor mais baixo e em data2 o
        // valor mais alto como por exemplo
        // data1=2016, data2=2023. Esta função verifica se a ordem esperada foi trocada
        // e ajusta para que não ocorra erros
        if (data1 >= data2) {
            Integer data = data2;
            data2 = data1;
            data1 = data;
        ...
        if (programa.isPresent()) {
            ...
            for (int i = 0; i < progRepo.getReferenceById(idPrograma).getDocentes().size(); i++) {
                ...
                if (progRepo.getReferenceById(idPrograma).getDocentes().get(i).getProducoes() != null
                    ...
                    for (int j = 0; j < progRepo.getReferenceById(idPrograma).getDocentes().get(i).getProducoes()
                            .size(); j++) {
                        if (progRepo.getReferenceById(idPrograma).getDocentes().get(i).getProducoes().get(j)
                                .getAno() >= data1
                                && progRepo.getReferenceById(idPrograma).getDocentes().get(i).getProducoes().get(j)
                                        .getAno() <= data2) {
                            producoes.add(
                                    progRepo.getReferenceById(idPrograma).getDocentes().get(i).getProducoes().get(j));
                        }
            ...
            if (producoes.isEmpty())
                throw new ServicoRuntimeException(
                        "O Programa não possui Docentes com Produções no periodo especificado");
     ...
     * for(int i = 0; i <
     * prodRepo.getReferenceById(idProducao).getOrientacoes().size(); i++){
     * retirarProducaoOrientacao(idProducao,
     * prodRepo.getReferenceById(idProducao).getOrientacoes().get(i).getId());
     * }
     * return true;
     * }
     ...
     * for(int i = 0; i <
     * prodRepo.getReferenceById(idProducao).getDocentes().size(); i++){
     * retirarProducaoDocente(idProducao,
     * prodRepo.getReferenceById(idProducao).getDocentes().get(i).getId());
     * }
     * return true;
     * }
